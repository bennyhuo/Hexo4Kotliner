...
Box.prototype._readDir = function(base, fn, prefix) {
  //console.log('readDir: %s', base);
  //这里需要过滤掉主题当中的 node_modules 目录
  if(base.indexOf('node_modules') >= 0){
    console.log(base.indexOf('node_modules'));
    return;
  }
  prefix = prefix || '';
...
Box.prototype.process = function(callback) {
  ...
  return fs.stat(base).then(function(stats) {
    ...
    ctx.log.debug('Box.process: %s', base);
    //if(base.indexOf("node_modules")) return;
...
Box.prototype._processFile = function(type, path) {
  ...
  return Promise.reduce(this.processors, function(count, processor) {
    ...
    //ctx.log.debug('Processing: %s, match path: %s', path, params);
